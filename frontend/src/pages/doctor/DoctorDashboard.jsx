    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="flex border-b mb-6 border-slate-200">
            <button onClick={() => { setActiveTab('appointments'); setActiveView('list'); }} className={`mr-4 pb-2 ${activeTab === 'appointments' ? 'border-b-2 border-blue-600 font-medium text-slate-700' : 'text-slate-400'}`}>Appointments</button>
            <button onClick={() => { setActiveTab('lab_requests'); setActiveView('list'); }} className={`mr-4 pb-2 ${activeTab === 'lab_requests' ? 'border-b-2 border-blue-600 font-medium text-slate-700' : 'text-slate-400'}`}>Lab Reports & Requests</button>
        </div>

        {activeTab === 'appointments' && activeView === 'list' && (
          <div className="bg-white shadow overflow-hidden sm:rounded-md p-6">
            <h3 className="text-lg font-medium text-slate-900 mb-4">Appointments</h3>
...
const LabRequestsList = ({ requests }) => {
    ...
    return (
        <div className="bg-white shadow overflow-hidden sm:rounded-md p-6">
            <h3 className="text-lg font-medium text-slate-900 mb-4">Lab Requests & Results</h3>
            {selectedReport ? (
                <div className="bg-slate-50 p-4 rounded border border-slate-200">
                    ...
                    <h4 className="font-bold text-xl text-slate-800">AI Diagnostic Report</h4>
                    <p className="text-sm text-slate-500">For Request ID: {selectedReport.lab_request_id}</p>
                    <div className="mt-4 p-4 bg-white border border-slate-200 rounded">
                        <p className="font-bold text-lg mb-2 text-slate-700">AI Analysis Result</p>
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-6 shadow rounded">
                ...
                <h3 className="text-xl font-bold mb-4 text-slate-900">Patient Consultation</h3>
                ...
                <h4 className="font-bold mt-6 mb-2 text-slate-800">Prescribe Medication</h4>
            ...
            <div className="bg-white p-6 shadow rounded h-fit">
                <h4 className="font-bold mb-4 text-slate-900">Request Lab Tests</h4>
                <p className="text-sm text-slate-500 mb-2">If condition is severe, request tests first. Review results later then prescribe.</p>
