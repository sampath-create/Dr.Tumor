        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
                <div className="bg-white shadow overflow-hidden sm:rounded-md p-6">
                    <h3 className="text-lg font-medium text-slate-900 mb-6">Pending Lab Requests</h3>
                    <ul className="divide-y divide-gray-200">
                        {requests.map(req => (
                             <li key={req.id} className="py-4">
                                <div className="flex justify-between items-start">
                                    <div>
                                        <p className="font-bold text-slate-800">Test: {req.test_type}</p>
                                        <p className="text-sm text-slate-500">Patient ID: {req.patient_id}</p>
                                        <p className="text-sm text-slate-500">Requested by Dr: {req.doctor_id}</p>
                                            ...
                                            {uploading === req.id ? (
                                                <div className="flex flex-col items-end gap-2">
                                                    ...
                                                    <div className="flex gap-2">
                                                        ...
                                                        <button onClick={() => handleUpload(req.id)} className="bg-blue-600 text-white px-2 py-1 rounded text-xs hover:bg-blue-700">Confirm Upload</button>
                                                    </div>
                                                </div>
                                            ) : (
                                                <button onClick={() => setUploading(req.id)} className="bg-blue-600 text-white px-3 py-1 rounded text-sm hover:bg-blue-700">
                                                    Upload Report
                                                </button>
                                            )}
                                        </div>
                                    ...
                                    {req.status === 'completed' && (
                                        <div className="text-right">
                                            <p className="text-xs text-slate-500">Report ID: {req.result_id}</p>
